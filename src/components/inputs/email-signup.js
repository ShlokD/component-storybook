...
export class EmailSignup extends Component {
  ...
  render() {
    const  signupButtonClass = `email-signup-submit ${this.state.submitted ? 'submitted' : ""}`;
    const text = this.state.submitted ? "Thank you!" : "Sign up now"
    return (
    <div className="email-signup-container">
      <input ref="emailField" className="email-signup-field" type="email" maxLength="50" />
      <button className={signupButtonClass} onClick={this._handleEmailSubmit}>{text}</button>
    </div>
    );
  }
}
