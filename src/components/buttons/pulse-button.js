...
export class PulseButton extends Component {
  ...
  _onMouseMove(ev) {
    ...
    const distance = Math.floor(Math.sqrt(Math.pow(xCoord, 2) + Math.pow(yCoord, 2))) / 100;
  ...
  render() {
    return (
      <div ref="pulse" className="pulse-button-container" onMouseMove={this._onMouseMove}>
        <span className={`pulse-button-text ${this.state.distance < 5 ? 'is-visible' : 'is-hidden'}`}>This is a pulsing button</span>
